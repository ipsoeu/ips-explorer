...
$(document).ready(function() {
  var related_cases_table = $("#related_cases").DataTable( { 
    "scrollY": "500px", "scrollCollapse": true, "paging": false, "dom": "Bfrtip", 
    "order": [[ 1, "desc" ]],
    "buttons": [ "copy", "csv", "excel", "pdf" ] 
  } );
  related_cases_table.columns().every( function () {
    var that = this;
  ...
  foreach ($d as $p) {
    if ($p["id"] != $id) {
      if ($technology == $p["technology"]) {
        $related_cases[$p["id"]]["technology"] = "Yes";
      }
      if ($geoextent == $p["geoextent"]) {
        $related_cases[$p["id"]]["geoextent"] = "Yes";
      }
      if (count(array_intersect($geocoverage, $p["geocoverage"])) > 0) {
        $related_cases[$p["id"]]["geocoverage"] = "Yes";
      }
      if ($primary_sector == $p["primary_sector"]) {
        $related_cases[$p["id"]]["primary_sector"] = "Yes";
      }
...
/*      
      if (count(array_intersect_assoc($unsdg, $p["unsdg"])) > 0) {
        $related_cases[$p["id"]]["unsdg"] = "Yes";
      }
*/      
      if (isset($related_cases[$p["id"]])) {
        $related_cases[$p["id"]]["name"] = $p["name"];
      }
    }
  }

//var_dump($related_cases);

  if (count($related_cases) > 0) {
    $html .= '<table id="related_cases" class="table table-hover table-striped">
<thead>
<tr>
<th>Service</th>
<th>Similarity</th>
...
<tbody>
';
    foreach ($related_cases as $rpi => $rp) {
//      $score = count($rp) - 1;
...
<tfoot>
<tr>
<th><input class="filter" type="text" placeholder="Filter by case" data-index="0" /></th>
